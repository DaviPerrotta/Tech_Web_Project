 ...
 window.onload = function(){
   referenceDb.on('value',(snapshot) => {
    ...
    function download_dbordini(db1){
        ...
        string_html= '<table id="customers"> <th>Ordini</th> <th>Orario</th> <th>€</th> </tr>'
        var i = 1
        ...
        for(var ordini in db_ordini){
            ...
            var costo_totale=0;
            string_html+= '<td>'+i+'</td> '+'<td>'+dt+'</td> '
        
            for(var articoli in db_ordini[ordini]){
               costo_totale+= parseInt( db_ordini[ordini][articoli]['prezzo'] )
            }
            string_html+= '<td>'+costo_totale+'</td> '
            i++
            string_html+= ' </tr>'
        }
        string_html+= '</table>'
